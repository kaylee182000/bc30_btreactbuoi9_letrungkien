...
export const bookingTicketReducer = (state = stateDefault, action) => {
  switch (action.type) {
    case "DAT_GHE": {
      ...
      let gheCanDat = listGheUpdate.find(
        (gheCanDat) => gheCanDat.soGhe === datGhe.soGhe
      );
      if (!gheCanDat) {
        listGheUpdate.push(datGhe);
      } else {
        listGheUpdate = listGheUpdate.filter(
          (gheDaCo) => gheDaCo.soGhe !== datGhe.soGhe
        );
      }
    ...
    case "HUY_GHE": {
      ...
      let listGheUpdate = [...state.listGheDangDat];
      listGheUpdate = listGheUpdate.filter(
        (gheCanHuy) => gheCanHuy.soGhe !== huyGhe.soGhe
      );
